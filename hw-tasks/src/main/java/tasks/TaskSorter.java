...
import graph.LabeledGraph.Nodes;
...
public class TaskSorter {
    ...
    private final LabeledGraph<Task, Dependency> G;
    ...
    public void addTask(Task t) {
        ...
        LabeledGraph<Task, Dependency>.Nodes node = G.new Nodes(t);
        if (!G.hasNode(node)) {
            G.addNode(node);
        }
    }
    ...
    public Set<Task> getTasks() {
        ...
        for(LabeledGraph<Task, Dependency>.Nodes n : G.getNodes()){
            tasks.add(n.getData());
        }
        return tasks;
    ...
    public void addDependency(Dependency dep) {
        ...
        LabeledGraph<Task, Dependency>.Nodes beforeN = G.new Nodes(before);
        LabeledGraph<Task, Dependency>.Nodes AfterN = G.new Nodes(after);

        if (!G.hasEdge(beforeN, AfterN, dep)) {
            G.addEdge(beforeN, AfterN, dep);
        }
    ...
    public Set<Dependency> getOutgoingDependencies(Task t) {
        ...
        LabeledGraph<Task, Dependency>.Nodes node = G.new Nodes(t);
        for (LabeledGraph<Task, Dependency>.Edges e : G.getEdges(node)) {
            dep.add(e.getLabel());
        }
        return dep;
    }
